...
Parser::Parser(string input)
{
	...
	// scanner somente trabalha com simbolos em nivel global
	// ou seja, externo as declaracoes do programa
}

void Parser::newTokenInit(int token, string lexeme)
{
	Token *t;

	t = new Token(token, lexeme);
	globalST->add(new STEntry(t, true));
}

void Parser::initSimbolTable()
{
	newTokenInit(CLASS, "class");
	newTokenInit(EXTENDS, "extends");
	newTokenInit(INT, "int");
	newTokenInit(STRING, "string");
	newTokenInit(BREAK, "break");
	newTokenInit(PRINT, "print");
	newTokenInit(READ, "read");
	newTokenInit(RETURN, "return");
	newTokenInit(SUPER, "super");
	newTokenInit(IF, "if");
	newTokenInit(ELSE, "else");
	newTokenInit(FOR, "for");
	newTokenInit(NEW, "new");
	newTokenInit(CONSTRUCTOR, "constructor");
}
